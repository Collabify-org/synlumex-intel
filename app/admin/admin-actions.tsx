...
export function AdminActions({ subscriptionId, currentPlanId, plans }: Props) {
  ...
  async function changePlan(newPlanId: string) {
    ...
    try {
      ...
      await supabase.from('subscription_history').insert({
        ...
        changed_by: user?.id,
        reason: 'Manual upgrade from admin panel'
      });
      ...
      setNotice(`Switched to ${plans.find((p) => p.id === newPlanId)?.name}`);
      router.refresh();
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function clearUsage() {
    ...
    try {
      ...
      if (error) throw error;
      setNotice('All usage events cleared');
      router.refresh();
    } catch (e: any) {
      setError(e.message);
    } finally {
          ...
          {plans.map((p) => {
            ...
            return (
              ...
              >
                ...
                {isCurrent ? `${p.name} (Current)` : `Upgrade to ${p.name}`}
              </Button>
            );
          })}
        </div>
